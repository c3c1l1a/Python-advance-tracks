...
class Sensei(MockableTestResult):
    ...
    def startTest(self, test):
        ...
        if helper.cls_name(test) != self.prevTestClassName:
            ...
            if not self.failures:
                self.stream.writeln()
                self.stream.writeln("{0}{1}Learning {2}".format(
    ...
    def learn(self):
        ...
        self.stream.writeln(
            "\n{0}**************************************************" \
            .format(Fore.RESET))
        self.stream.writeln("\n{0}That was the last one for today, well done!" \
            .format(Fore.MAGENTA))
        self.stream.writeln(
            "\nDon't stop learning, though!!")
    ...
    def errorReport(self):
        ...
        self.stream.writeln("  {0}{1}{2} "
          "".format(Fore.RED, Style.BRIGHT, test._testMethodDoc))

        self.stream.writeln("\n{0}{1}Seems you have not yet completed the activity ..." \
    ...
    def report_progress(self):
        return "You have completed {0} activities " \
            "in this lesson.".format(
                self.pass_count)
    ...
    def say_something_zenlike(self):
        if self.failures:
            ...
            return '"{0}Testing shows the presence, not the absence of bugs. Edsger W. Dijkstra"' \
                .format(Fore.CYAN)
